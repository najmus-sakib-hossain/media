...
use dx_media::scraping::{ScrapingRegistry, ScrapingCategory};
use dx_media::tools::{image, document, archive, utility};
...
fn setup_dirs() {
    ...
    for p in ["openverse", "wikimedia", "met", "nasa", "archive", "loc", "dpla", 
              "rijksmuseum", "cleveland", "europeana", "polyhaven", "artic", "picsum"] {
...
mod dxmedia_tests {
    ...
    fn test_dxmedia_initialization() {
        ...
        match DxMedia::new() {
            Ok(dx) => {
                ...
                for provider in &all {
                    let status = if dx.is_provider_available(provider) { "✓" } else { "✗" };
    ...
    async fn test_dxmedia_search_wikimedia() {
        ...
        match DxMedia::new() {
            Ok(dx) => {
                let result = dx.search("nature landscape")
    ...
    async fn test_dxmedia_search_nasa() {
        ...
        match DxMedia::new() {
            Ok(dx) => {
                let result = dx.search("earth space")
                    .media_type(MediaType::Image)
                    .provider("nasa")
                    .count(3)
                    .execute()
    ...
    async fn test_dxmedia_search_met() {
        ...
        match DxMedia::new() {
            Ok(dx) => {
                let result = dx.search("painting art")
                    .media_type(MediaType::Image)
                    .provider("met")
                    .count(3)
                    .execute()
...
mod tool_tests {
    ...
    fn test_image_tools_suite() {
        ...
        if !test_image.exists() {
            ...
            let png_bytes: &[u8] = &[
                ...
                0x08, 0x02, 0x00, 0x00, 0x00, 0x4B, 0x6D, 0x29, 0xDE,
        ...
        println!("8. Info (EXIF): {}", if r.is_ok() { "✓" } else { "✗ (needs exiftool)" });
    ...
    fn test_document_tools_suite() {
        ...
        fs::write(&md_file, "# Test Document\n\nThis is a **test** paragraph.\n\n- Item 1\n- Item 2").ok();
        ...
        println!("6. HTML to PDF: {}", if r.is_ok() { "✓" } else { "✗ (needs wkhtmltopdf)" });
        
        // 7. Document Convert
        let out = output_dir.join("converted.txt");
        let r = document::convert_document(&md_file, &out, document::DocFormat::Txt);
        println!("7. Document Convert: {}", if r.is_ok() { "✓" } else { "✗ (needs pandoc)" });
