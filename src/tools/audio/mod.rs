...
impl AudioTools {
    ...
    pub fn convert<P: AsRef<Path>>(&self, input: P, output: P, options: ConvertOptions) -> Result<super::ToolOutput> {
        converter::convert_audio(input, output, options)
    }
    
    /// Normalize audio levels.
    pub fn normalize<P: AsRef<Path>>(&self, input: P, output: P, options: NormalizeOptions) -> Result<super::ToolOutput> {
    ...
    pub fn spectrum<P: AsRef<Path>>(&self, input: P, output: P, options: SpectrumOptions) -> Result<super::ToolOutput> {
    ...
    pub fn remove_silence<P: AsRef<Path>>(&self, input: P, output: P, options: SilenceOptions) -> Result<super::ToolOutput> {
        silence::remove_silence(input, output, options)
    }
    
    /// Split audio file.
    pub fn split<P: AsRef<Path>>(&self, input: P, output_dir: P, options: SplitOptions) -> Result<super::ToolOutput> {
        splitter::split_audio(input, output_dir, options)
    }
    
    /// Apply audio effect.
    pub fn apply_effect<P: AsRef<Path>>(&self, input: P, output: P, effect: AudioEffect) -> Result<super::ToolOutput> {
