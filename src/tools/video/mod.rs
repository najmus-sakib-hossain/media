...
impl VideoTools {
    ...
    pub fn transcode<P: AsRef<Path>>(&self, input: P, output: P, options: TranscodeOptions) -> Result<super::ToolOutput> {
        transcoder::transcode_video(input, output, options)
    }
    
    /// Extract audio from video.
    pub fn extract_audio<P: AsRef<Path>>(&self, input: P, output: P, format: AudioFormat) -> Result<super::ToolOutput> {
        audio_extract::extract_audio(input, output, format)
    }
    
    /// Trim video to specified duration.
    pub fn trim<P: AsRef<Path>>(&self, input: P, output: P, start: f64, end: f64) -> Result<super::ToolOutput> {
        trimmer::trim_video(input, output, start, end)
    }
    
    /// Convert video to GIF.
    pub fn to_gif<P: AsRef<Path>>(&self, input: P, output: P, options: GifOptions) -> Result<super::ToolOutput> {
        gif_maker::video_to_gif(input, output, options)
    }
    
    /// Extract thumbnail from video.
    pub fn thumbnail<P: AsRef<Path>>(&self, input: P, output: P, timestamp: f64) -> Result<super::ToolOutput> {
        thumbnail::extract_thumbnail(input, output, timestamp)
    }
    
    /// Scale video resolution.
    pub fn scale<P: AsRef<Path>>(&self, input: P, output: P, width: u32, height: u32) -> Result<super::ToolOutput> {
        scaler::scale_video(input, output, width, height)
    }
    
    /// Concatenate multiple videos.
    pub fn concatenate<P: AsRef<Path>>(&self, inputs: &[P], output: P) -> Result<super::ToolOutput> {
    ...
    pub fn add_watermark<P: AsRef<Path>>(&self, input: P, output: P, text: &str) -> Result<super::ToolOutput> {
        watermark::add_text_watermark(input, output, text)
    }
    
    /// Change video playback speed.
    pub fn change_speed<P: AsRef<Path>>(&self, input: P, output: P, factor: f32) -> Result<super::ToolOutput> {
        speed::change_speed(input, output, factor)
    }
    
    /// Burn subtitles into video.
    pub fn burn_subtitles<P: AsRef<Path>>(&self, input: P, subtitles: P, output: P) -> Result<super::ToolOutput> {
