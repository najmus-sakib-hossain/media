...
/// Extract using unrar.
fn extract_with_unrar(input: &Path, output_dir: &Path, password: Option<&str>) -> Result<ToolOutput> {
...
pub fn list_rar_with_password<P: AsRef<Path>>(input: P, password: Option<&str>) -> Result<ToolOutput> {
    let input_path = input.as_ref();
...
pub fn test_rar_with_password<P: AsRef<Path>>(input: P, password: Option<&str>) -> Result<ToolOutput> {
    let input_path = input.as_ref();
