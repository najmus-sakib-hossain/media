...
fn create_with_zip<P: AsRef<Path>>(inputs: &[P], output: &Path, options: &ZipOptions) -> Result<ToolOutput> {
    let mut cmd = Command::new("zip");
    cmd.arg("-r")
        .arg(format!("-{}", options.level.level()));
...
fn create_with_7z<P: AsRef<Path>>(inputs: &[P], output: &Path, options: &ZipOptions) -> Result<ToolOutput> {
...
fn extract_with_unzip(input: &Path, output_dir: &Path, password: Option<&str>) -> Result<ToolOutput> {
